...
| Primary launcher ownership | Native Go | start.bat defaults to Go-primary when binary exists ("./hypernexus.exe"). Use --runtime node for TS-primary. |
...
| Imported session storage | Partial Native Go | Go now has a real imported-session store plus native file-based and DB-backed ingest paths (`llm-cli` logs.db and HyperNexus DB artifacts), with transcript-hash dedup, archived transcript persistence, memory rows, and instruction-doc regeneration; some niche parser parity gaps still remain |
